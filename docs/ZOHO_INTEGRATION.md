...
- `/api/zoho` сейчас открыт (любой знающий URL может запросить данные по email). **TODO до продакшена:** закрыть эндпоинт проверкой сессии Supabase (пускать только залогиненного staff/admin). Пометка в коде.
...
### Обязательная защита эндпоинта (до продакшена)
Сейчас `/api/zoho?email=...` открыт — любой мог бы запросить чужие данные. Для модели «каждый свой» нужно закрыть:
- Фронт шлёт **access token** сессии Supabase в заголовке `Authorization`.
- `/api/zoho` проверяет токен, определяет email из профиля и **игнорирует email из query** (кроме админов, которым можно смотреть чужие).
- Роль из `profiles.role`: `admin` → любой email; staff-роли → только собственный.
...
1. Подключить реальные ключи Zoho (разделы 2–3). ← **сейчас**
2. Защитить `/api/zoho` сессией Supabase (email из токена, не из query).
3. В `/partner` показать кабинет текущего пользователя (его Zoho-данные).
4. Роли: admin — вся команда; staff — только свой аккаунт.
