// Serverless-функция для Vercel: защищённый прокси к Google Gemini.
// Ключ живёт на сервере (env GEMINI_API_KEY), в браузер не попадает.
// Фронт вызывает: fetch('/api/advisor', {method:'POST',
//   body: JSON.stringify({ prompt, context, lang })})
//   context — база знаний платформы (BetConstruct + Marketplace + Партнёры).

function systemPrompt(lang, context) {
  const langName = lang === 'en' ? 'English' : lang === 'hy' ? 'Armenian (Հայերեն)' : 'Russian';
  return `Ты — AI-советник платформы BetConstructAI (B2B iGaming-маркетплейс вокруг экосистемы BetConstruct).

СТРОГОЕ ПРАВИЛО: отвечай ТОЛЬКО на основе данных платформы ниже (раздел «ДАННЫЕ ПЛАТФОРМЫ»).
В них передан весь контент сайта: (1) BETCONSTRUCT — iGaming Solutions с пометками «для чего», «кому подходит»
и «чем удобно», платформы и конструкторы с ценами, пакеты, продукты по категориям с ценами, Backoffice/Spring BME,
регионы, лицензии, платежи, loyalty-программа, демо-ссылки на живые продукты; (2) КАРТА САЙТА — какие разделы
и формы есть на платформе и что в них заполняется; (3) MARKETPLACE — продукты вендоров; (4) ПАРТНЁРЫ — производители.
Не выдумывай продукты, цены, компании или лицензии, которых нет в данных. Если чего-то нет — честно скажи,
что этого пока нет в каталоге, и предложи связаться с менеджером.

Задача: по запросу клиента (гео, вертикаль, крипто/фиат, тип игр, бюджет, есть ли лицензия) собери конфигурацию:
— модель сотрудничества из iGaming Solutions (Turnkey / White Label / Crypto / API) и объясни, почему именно она —
  опирайся на «кому подходит» и «что нужно от партнёра» из данных;
— рекомендованный регион/юрисдикция и лицензия (из данных);
— продукт/пакет/платформа BetConstruct с ценой (из данных, цены приводи дословно);
— конкретные продукты вендоров из MARKETPLACE (по именам) под задачу;
— подходящих партнёров-производителей по именам;
— платежи (крипто/фиат) из данных.
Где уместно — подскажи, в каком разделе платформы клиент найдёт детали (по КАРТЕ САЙТА), и дай демо-ссылку,
если она есть в данных. Ссылайся на конкретные названия из данных. Отвечай кратко и структурировано на языке: ${langName}.
Пиши обычным текстом без markdown-разметки (звёздочки, решётки) — ответ выводится как есть.
В конце добавь: «Предварительная рекомендация, не юридическая консультация — финал с менеджером.»

===== ДАННЫЕ ПЛАТФОРМЫ =====
${context || '(данные не переданы)'}
===== КОНЕЦ ДАННЫХ =====`;
}

export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
  if (req.method === 'OPTIONS') return res.status(200).end();
  if (req.method !== 'POST') return res.status(405).json({ error: 'POST only' });

  const key = process.env.GEMINI_API_KEY;
  if (!key) return res.status(500).json({ error: 'GEMINI_API_KEY not set' });

  let prompt = '', context = '', lang = 'ru';
  try {
    const b = req.body || {};
    prompt = b.prompt || '';
    context = b.context || '';
    lang = b.lang || 'ru';
  } catch (e) {}
  if (!prompt) return res.status(400).json({ error: 'prompt required' });

  const MODELS = ['gemini-3.5-flash', 'gemini-2.5-flash-lite', 'gemini-pro-latest', 'gemini-flash-lite-latest'];
  const errs = [];
  for (const model of MODELS) {
    try {
      const url = 'https://generativelanguage.googleapis.com/v1beta/models/' + model + ':generateContent?key=' + key;
      const r = await fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: systemPrompt(lang, context) }] },
          contents: [{ parts: [{ text: prompt }] }],
          generationConfig: { temperature: 0.4 }
        })
      });
      const data = await r.json();
      const text = data && data.candidates && data.candidates[0] && data.candidates[0].content
        && data.candidates[0].content.parts && data.candidates[0].content.parts[0]
        && data.candidates[0].content.parts[0].text;
      if (text) return res.status(200).json({ text, model });
      const em = (data && data.error && data.error.message)
        || (data && data.candidates && data.candidates[0] && data.candidates[0].finishReason)
        || (data && data.promptFeedback && data.promptFeedback.blockReason)
        || ('HTTP ' + r.status);
      errs.push(model + ' → ' + em);
    } catch (e) {
      errs.push(model + ' → ' + String(e));
    }
  }
  // все модели не сработали — узнаём, что вообще доступно этому ключу
  let available = '';
  try {
    const lm = await fetch('https://generativelanguage.googleapis.com/v1beta/models?key=' + key);
    const lmj = await lm.json();
    if (lmj && lmj.models) {
      available = lmj.models
        .filter(m => (m.supportedGenerationMethods || []).indexOf('generateContent') !== -1)
        .map(m => m.name).join(', ');
    } else if (lmj && lmj.error) {
      available = 'ListModels error: ' + lmj.error.message;
    }
  } catch (e) { available = 'ListModels failed: ' + String(e); }
  return res.status(200).json({ text: 'Не удалось получить ответ.', errors: errs, availableModels: String(available).slice(0, 800) });
}
